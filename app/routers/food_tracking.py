...
@router.post("/checkin", response_model=Food_TrackingRead, status_code=status.HTTP_201_CREATED)
def checkin(*,
              session: SessionDep,
              food_tracking: Food_TrackingCreate):
...
def getmeals(*,
    ...
    query = select(Food_Tracking, Meal).\
        join(Meal).\
        where(Food_Tracking.user_id == user_id)
...
def getusers(*,
    ...
    query = select(Food_Tracking, Meal).\
        join(Meal).\
        where(Food_Tracking.meal_id == meal_id)
...
def gettracking(*,
            session: SessionDep,tracking_id: UUID):

    query = select(Food_Tracking, Meal).\
        where(Food_Tracking.id == tracking_id)
